...
stocks_index = {
    ...
    "印度": {
        ...
        "row": 0,
        "result": "",
        "url": "https://hq.sinajs.cn/list=znb_",
    },
    "德国": {
        "code": "DAX",
        "row": 0,
    ...
    for name, data in stocks_index.items():
        ...
        try:
            ...
            if "qt.gtimg.cn" in url:
                ...
                # 解析 sina 的响应
                data["result"] = response.text.split(",")[1]
            print(f"{name}: {data.get('result')}")
            if data["row"] == 4:
                # 写入日期和标题
                ws.cell(
                    row=1, column=target_col, value=datetime.now().strftime("%Y/%m/%d")
                )
...
def update_pe_pb_xilv_to_ws(ws, target_col):
    for name, data in pe_pb_xilv.items():
        if data["row"] == 0:
            continue
        point, pe, pb, xilv = fetch_pe_pb_xilv_data(
            data["code"], int(time.time() * 1000)
        )
        # 结果按 calc 权重计算，并保留两位小数
        try:
            result = (
                pe * data["calc"][0] + pb * data["calc"][1] + xilv * data["calc"][2]
            )
